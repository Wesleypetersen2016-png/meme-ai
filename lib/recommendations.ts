...
export function goalRecommendations(tokens: Token[], goals: GoalProfile): Recommendation[] {
  const riskPenalty = goals.riskTolerance === "conservative" ? 0.75 : goals.riskTolerance === "balanced" ? 0.45 : 0.2;
  return tokens
    .filter(token => goals.chains.includes(token.chain))
    .map(token => {
      const momentum = Math.max(-20, Math.min(35, token.change ?? 0));
      const objectiveBoost = goals.objective === "growth" ? momentum * 0.45 : goals.objective === "preservation" ? (100 - token.riskScore) * 0.15 : 4;
      const horizonBoost = goals.horizon === "intraday" ? Number(token.buySellRatio ?? 1) * 4 : goals.horizon === "long-term" ? (100 - token.riskScore) * 0.08 : token.convictionScore * 0.05;
      const fitScore = clamp(token.convictionScore + objectiveBoost + horizonBoost - token.riskScore * riskPenalty);
