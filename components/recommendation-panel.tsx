...
export function RecommendationPanel({ data, limit = 5, loading = false }: { data: Token[]; limit?: number; loading?: boolean }) {
  ...
  return <section className="panel overflow-hidden">
    ...
    <div className="divide-y divide-[var(--line)]">{recommendations.map(({ token, action, fitScore, allocation, reasons, projection }) => {
      ...
      return <article key={id} className="recommendation-row group">
        ...
        <div className={`recommendation-details ${isOpen ? "is-open" : ""}`}>
          <div className="grid gap-5 px-5 pb-5 md:grid-cols-[1.15fr_.85fr]">
            <div className="rounded-xl border border-[var(--line)] bg-[#0e1014] p-4"><div className="eyebrow">Why this rank</div><ul className="mt-3 grid gap-2 text-xs leading-5 text-[#a2a7b0]">{reasons.map(reason => <li key={reason} className="flex gap-2"><span className="text-[#656c77]">—</span><span>{reason}</span></li>)}</ul>{action === "Consider" && <p className="mt-4 border-t border-[var(--line)] pt-3 text-xs text-[#cbd0d6]">Your current limit suggests no more than <strong className="text-white">{allocation}%</strong> of your portfolio.</p>}</div>
            <div className="rounded-xl border border-[var(--line)] bg-[#0e1014] p-4"><div className="flex items-center justify-between gap-3"><div className="eyebrow">Modeled {projection.timeframe} scenario</div><span className="text-[10px] text-[#777e89]">{projection.confidence} confidence</span></div><div className="mt-5 grid grid-cols-3 gap-2 text-center"><div><div className="mono text-sm text-[#c69ba2]">{signed(projection.bear)}</div><div className="mt-2 text-[10px] text-[#6f7580]">Bear</div></div><div><div className="mono text-lg font-semibold text-white">{signed(projection.base)}</div><div className="mt-2 text-[10px] text-[#6f7580]">Base</div></div><div><div className="mono text-sm text-[#9bc4a8]">{signed(projection.bull)}</div><div className="mt-2 text-[10px] text-[#6f7580]">Bull</div></div></div><p className="mt-4 text-[10px] leading-4 text-[#747b86]">A momentum scenario calculated from current market inputs—not a guaranteed price forecast.</p>{token.pairUrl && <a href={token.pairUrl} target="_blank" rel="noreferrer" onClick={event => event.stopPropagation()} className="mt-3 inline-flex text-xs font-medium text-[var(--accent)]">View live market</a>}</div>
          </div>
        </div>
      </article>;
