...
export function TokenTable({ compact = false, data, title = "Market intelligence", description = "Live market data ranked by liquidity, momentum, and modeled risk", emptyMessage = "No live tokens found." }: { compact?: boolean; data: Token[]; title?: string; description?: string; emptyMessage?: string }) {
  const [query, setQuery] = useState("");
  const [chain, setChain] = useState("All chains");
  const chains = useMemo(() => [...new Set(data.map(token => token.chain))].sort(), [data]);
  const rows = useMemo(() => data.filter(t => (chain === "All chains" || t.chain === chain) && `${t.name} ${t.symbol} ${t.address ?? ""}`.toLowerCase().includes(query.toLowerCase())), [query, chain, data]);
  return (
    <section className="panel overflow-hidden">
      ...
      <div className="overflow-x-auto mobile-scroll">
        <table className="w-full min-w-[1180px] text-left">
          <thead><tr className="eyebrow border-b border-[#272b27]">{["Token", "Chain", "Market cap / FDV", "Liquidity", "Volume 5m / 1h", "Buy / Sell", "Social IQ", "Modeled risk", "Modeled IQ"].map(h => <th key={h} className="px-4 py-3.5 font-medium first:pl-5">{h}</th>)}</tr></thead>
          <tbody>
            ...
            {!rows.length && <tr><td colSpan={9} className="px-5 py-10 text-center text-sm text-[#777e77]">{emptyMessage}</td></tr>}
          </tbody>
        </table>
      </div>
    </section>
